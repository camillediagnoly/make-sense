...
const EditShortcutsPopup: React.FC<IProps> = ({
  ...
  const handleKeyDown = (event: KeyboardEvent) => {
    ...
    const key = event.key;
    if (
      !["Control", "Alt", "Shift", "Meta", "Escape"].includes(key) &&
    ...
    return (
      <div className="EditShortcutsPopupContent">
        <div className="Message">
          Edit keyboard shortcuts by clicking on a shortcut and pressing the new
          key combination.
        </div>
            ...
            <div key={shortcut.id} className="ShortcutItem">
              ...
              <div
                className={`ShortcutKeys ${
                  activeShortcut === shortcut.id ? "recording" : ""
                }`}
  ...
  const onAccept = () => {
    BrowserSettingsStorage.saveKeyboardShortcuts(localShortcuts);
    ...
    localShortcuts.forEach((shortcut) => {
      ...
      if (
        original &&
        JSON.stringify(original.keyCombo) !== JSON.stringify(shortcut.keyCombo)
      ) {
...
export default connect(mapStateToProps, mapDispatchToProps)(EditShortcutsPopup);
